...
def tuneThresholdfromScore(scores, labels, target_fa, target_fr = None):
	...
	fpr, tpr, thresholds = metrics.roc_curve(labels, scores, pos_label=1)#fpr被错误分类的负样本的比例
    #fpr错误接收率，tpr错误召回率
	fnr = 1 - tpr#错误拒绝lv
	tunedThreshold = [];
	if target_fr:
...
def ComputeErrorRates(scores, labels):
      ...
      sorted_indexes, thresholds = zip(*sorted(
          ...
          key=itemgetter(1)))#按照元组的第二个顺序进行排序
      sorted_labels = []
      ...
      for i in range(0, len(labels)):
          if i == 0:
              fnrs.append(labels[i])#正确接收的数组
              fprs.append(1 - labels[i])#错误接收的数组
          else:
